...
def plan_day(
    ...
    food_options = [e for e in establishments if e.kind in (EstablishmentKind.RESTAURANT, EstablishmentKind.COFFEE_SHOP)]
    shop_options = [e for e in establishments if e.kind in (EstablishmentKind.SUPERMARKET, EstablishmentKind.CLOTHING, EstablishmentKind.HARDWARE, EstablishmentKind.PHARMACY)]
    leisure_options = [e for e in establishments if e.kind in (EstablishmentKind.PARK, EstablishmentKind.PUB)]
    ...
    if work_dest_cell is not None:
        ...
        intentions.append(Intention(start_minute=commute_start, activity=Activity.COMMUTE, cell=work_dest_cell))
        work_start = commute_start + 30
        intentions.append(Intention(start_minute=work_start, activity=work_activity, cell=work_dest_cell))
        ...
        if lunch_target is not None:
            ...
            intentions.append(Intention(start_minute=lunch_start, activity=Activity.EAT, cell=lunch_target.cell))
            intentions.append(Intention(start_minute=lunch_start + 45, activity=work_activity, cell=work_dest_cell))
        ...
        if rng.random() < 0.4 and shop_options:
            ...
            intentions.append(Intention(start_minute=end_of_work, activity=Activity.COMMUTE, cell=shop_target.cell))
            intentions.append(Intention(start_minute=end_of_work + 20, activity=Activity.SHOP, cell=shop_target.cell))
            head_home = end_of_work + 50
        else:
            head_home = end_of_work
        intentions.append(Intention(start_minute=head_home, activity=Activity.COMMUTE, cell=agent.home_cell))
        intentions.append(Intention(start_minute=head_home + 30, activity=Activity.LEISURE, cell=agent.home_cell))
    else:
        ...
        if rng.random() < 0.5 and leisure_options:
            ...
            intentions.append(Intention(start_minute=t, activity=Activity.COMMUTE, cell=target.cell))
            intentions.append(Intention(start_minute=t + 25, activity=Activity.LEISURE, cell=target.cell))
            intentions.append(Intention(start_minute=t + 25 + 90, activity=Activity.COMMUTE, cell=agent.home_cell))
    ...
    if rng.random() < 0.25 and food_options:
        ...
        intentions.append(Intention(start_minute=dinner_start, activity=Activity.COMMUTE, cell=dinner_cell))
        intentions.append(Intention(start_minute=dinner_start + 20, activity=Activity.EAT, cell=dinner_cell))
        intentions.append(Intention(start_minute=dinner_start + 75, activity=Activity.COMMUTE, cell=agent.home_cell))
    else:
        intentions.append(Intention(start_minute=dinner_start, activity=Activity.EAT, cell=agent.home_cell))

    # Sleep
    sleep_time = 22 * 60 + int(rng.integers(0, 90))
    intentions.append(Intention(start_minute=sleep_time, activity=Activity.SLEEP, cell=agent.home_cell))
